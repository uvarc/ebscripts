...
""" % {'lic': self.license_file, 'ins': self.installdir}
    ...
    def sanity_check_step(self):
        ...
        if impi_ver >= LooseVersion('2021.11'):
            ...
            version_parts.pop()
            mpi_subdir = os.path.join('mpi', '.'.join(version_parts))
        ...
        elif impi_ver >= LooseVersion('2021'):
        ...
        if impi_ver >= LooseVersion('2021.11'):
            custom_paths = {
                ...
                [os.path.join(include_dir, 'mpi', x) for x in mpi_mods] +
        ...
        if build_option('mpi_tests'):
            if impi_ver >= LooseVersion('2017'):
                ...
                if impi_ver >= LooseVersion('2021.11'):
                    version_parts = self.version.split('.')
                    version_parts.pop()
                    impi_testsrc = os.path.join(
                        self.installdir, 'mpi', '.'.join(version_parts),
                        'opt', 'mpi', 'test', 'test.c'
                    )
                elif impi_ver >= LooseVersion('2021'):
    ...
    def make_module_req_guess(self):
        ...
        if self.cfg['m32']:
            ...
            if impi_ver >= LooseVersion('2021'):
                ...
                include_dirs = [
                    os.path.join(mpi_subdir, 'include'),
                    os.path.join(mpi_subdir, 'include', 'mpi')
                ]
                path_dirs = [
    ...
    def make_module_extra(self, *args, **kwargs):
        ...
        if LooseVersion(self.version) >= LooseVersion('2021.11'):
            version_parts = self.version.split('.')
            version_parts.pop()
            mpiroot = os.path.join(self.installdir, 'mpi', '.'.join(version_parts))
