...
class EB_imkl(IntelBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if LooseVersion(self.version) >= LooseVersion('2024'):
            ...
            version_parts.pop()
            self.mkl_basedir = os.path.join('mkl', '.'.join(version_parts))
    ...
    def build_mkl_fftw_interfaces(self, libdir):
        ...
        if loosever >= LooseVersion('2024'):
            intsubdir = os.path.join(self.mkl_basedir, 'share', 'mkl', 'interfaces')
            ...
            if self.cfg['m32']:
                ...
                if LooseVersion(self.version) >= LooseVersion('2021'):
                    if LooseVersion(self.version) >= LooseVersion('2024'):
                        version_parts = self.version.split('.')
                        version_parts.pop()
                        compiler_subdir = os.path.join('compiler', '.'.join(version_parts), 'linux', 'compiler', 'lib', 'intel64_lin')
                    else:
    ...
    def make_module_extra(self):
        ...
        if LooseVersion(self.version) >= LooseVersion('2024'):
            version_parts = self.version.split('.')
            version_parts.pop()
            mklroot = os.path.join(self.installdir, 'mkl', '.'.join(version_parts))
        elif LooseVersion(self.version) >= LooseVersion('2021'):
