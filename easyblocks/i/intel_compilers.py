...
class EB_intel_minus_compilers(IntelBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if LooseVersion(self.version) >= LooseVersion('2024.0'):
            ...
            version_parts.pop()
            self.compilers_subdir = os.path.join('compiler', '.'.join(version_parts))
        else:
    ...
    def sanity_check_step(self):
        ...
        if LooseVersion(self.version) >= LooseVersion('2024.0'):
            all_compiler_cmds = oneapi_compiler_cmds
        else:
        ...
        if LooseVersion(self.version) >= LooseVersion('2024.0'):
            custom_commands.extend("%s --version" % c for c in oneapi_compiler_cmds)
        elif LooseVersion(self.version) >= LooseVersion('2022.0'):
    ...
    def make_module_req_guess(self):
        ...
        guesses = {
            ...
            'MANPATH': [
                os.path.join('compiler', 'latest', 'documentation', 'en', 'man', 'common'),
            ],
            'OCL_ICD_FILENAMES': [
