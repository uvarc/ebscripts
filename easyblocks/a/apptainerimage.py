        ...
        if self.cfg['staged_install']:
            ...
            try:
                ...
                raise EasyBuildError(
                    "Failed to move staged install from %s to %s: %s", staged_installdir, self.installdir, err)

        for srcfile in self.cfg['sources']:
            if srcfile.endswith('.sif') or srcfile.endswith('.simg') or srcfile.endswith('.img'):
                containerfile = srcfile
                break
        os.system('mv -f %s/%s %s' %
                  (self.installdir, containerfile, container_dir))
        ...
        if deffile is not None:
            ...
            try:
                ...
                shutil.copy(os.path.join(
                    self.cfg['start_dir'], deffile), easybuilddir)
            except OSError as err:
                raise EasyBuildError(
                    "Failed to copy %s to %s: %s", self.cfg['deffile'], easybuilddir, err)
